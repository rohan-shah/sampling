...
namespace sampling
{
	void fearnheadGetKappa(std::vector<mpfr_class>& sortedWeights, boost::mt19937& randomSource, int N, int& A, mpfr_class& B)
	{
		...
		std::size_t nUnits = sortedWeights.size();
		int lowerBound = 0, upperBound = (int)nUnits-1;
		mpfr_class lowerBoundSum = 0;
		const mpfr_class tolerance = 1e-8;
		while(true)
		{
			//Split the existing partition between currentIndex and currentIndex2
			int currentIndex = (lowerBound + upperBound - 1) / 2;
			...
			if(currentIndex == (int)nUnits - 1)
			{
				mpfr_class partitionWeight = sortedWeights[currentIndex];
				A = (int)nUnits - lowerBound;
				B = lowerBoundSum;
				int i = lowerBound;
				for(; i < (int)nUnits && sortedWeights[i] < partitionWeight; i++)
			...
			{
				mpfr_class partitionWeight1 = sortedWeights[currentIndex], partitionWeight2 = sortedWeights[currentIndex+1];
				mpfr_class B1 = lowerBoundSum;
				int A1 = (int)nUnits - lowerBound;
				int i = lowerBound;
				for(; sortedWeights[i] < partitionWeight1 && i < (int)nUnits; i++)
				{
			...
			if(sortedWeights[lowerBound] == sortedWeights[upperBound])
			{
				mpfr_class partitionWeight = sortedWeights[lowerBound];
				B = lowerBoundSum;
				A = (int)nUnits - lowerBound;
				int i = lowerBound;
				for(; sortedWeights[i] < partitionWeight; i++)
